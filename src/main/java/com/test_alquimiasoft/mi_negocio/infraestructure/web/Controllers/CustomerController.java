package com.test_alquimiasoft.mi_negocio.infraestructure.web.Controllers;
...
import com.test_alquimiasoft.mi_negocio.infraestructure.web.DTO.*;
...
public class CustomerController {
    ...
    public List<CustomerDto> search(@RequestParam(required = false) String query) {
        return service.search(query)
            ...
            .map(c -> {
                ...
                List<AddressDto> extra = Optional.ofNullable(c.getExtraAddresses())
    ...
    public ResponseEntity<CustomerDto> create(@Valid @RequestBody CustomerCreateRequest req) {
        Address mat = new Address(
            UUID.randomUUID(),
            req.getAddress().getProvince(),
            req.getAddress().getCity(),
            req.getAddress().getAddressLine(),
            req.getAddress().isMain()
        );
        Customer domain = new Customer(
            UUID.randomUUID(),
            req.getIdentificationType(),
        ...
        List<AddressDto> extra = Optional.ofNullable(saved.getExtraAddresses())
            .orElse(Collections.emptySet())
            .stream()
            .map(a -> new AddressDto(
                a.getId(),
    ...
    public CustomerDto update(
        ...
        List<AddressDto> extra = Optional.ofNullable(updated.getExtraAddresses())
            .orElse(Collections.emptySet())
            .stream()
            .map(a -> new AddressDto(
                a.getId(),
